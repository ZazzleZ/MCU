...
const keyedSelection = computed(() => {
  const selectedKeys = new Set();
  props.selectedRows.forEach((row) => {
    if (rowHasKey(row)) selectedKeys.add(row[props.rowKey]);
  });
  return selectedKeys;
});

const isRowSelected = (row) => {
  if (rowHasKey(row)) return keyedSelection.value.has(row[props.rowKey]);
  return props.selectedRows.includes(row);
};
    ...
    <table class="min-w-full border-collapse text-sm text-slate-700">
      ...
      >
        <tr>
          ...
          >
            <!-- Wenn es die Actions-Spalte ist und Slot vorhanden, leere Überschrift -->
            <template v-if="lastColumn && column.key === lastColumn.key && slots.actions"></template>

            <!-- NEU: Optionaler Spalten-Header-Slot: #header-<key> -->
            <template v-else>
      ...
      <tbody class="divide-y divide-slate-200">
        ...
        >
          ...
          >
            <!-- Actions-Slot bleibt wie gehabt -->
            <slot
              v-if="lastColumn && column.key === lastColumn.key && slots.actions"
              name="actions"
              :row="row"
            ...
            <!-- NEU: Dynamischer Spaltenslot #<key> (z. B. #grafik_url) -->
            <slot
